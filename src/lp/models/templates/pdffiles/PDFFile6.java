...
import lp.models.templates.PDFsMetrics.PDFFile6.*;
import lp.models.templates.PDFsMetrics.StudentLARectangle;
import org.apache.pdfbox.pdmodel.PDPage;
import org.apache.pdfbox.pdmodel.PDPageContentStream;

public class PDFFile6 extends PDFFile123456 {
    
    public PDFFile6() {
        super();
        tpl = tempaltesDir + "Template.6.pdf";
        pages[0] = new Page1();
        pages[1] = new Page2();
        pages[2] = new Page3();
        pages[3] = new Page4();
    ...
    protected void add1PageGroup(Group g, PDPage page, int start, int stuCount, BasePage metrics) {
        ...
        try {
            try (PDPageContentStream pcs = new PDPageContentStream(doc, page, PDPageContentStream.AppendMode.APPEND, true, true)) {
                ...
                placeBidirectionalString(metrics.getTitle(), pcs, g.getName(), Settings.PREF_TEMPORARY.get("TPL6_TITLE"));
                placeString(metrics.getCustomMsg1(), pcs, Settings.PREF_TEMPORARY.get("TPL6_CUSTOM_STRING"));
                for ( int i = start, j = 0; i < stuCount; i++, j++ ) {
                    placeStudent(metrics.getStuRect(), pcs, g.getStudent(i), j, getAlignment(Settings.PREF_TEMPORARY.get("TPL6_NAMES_ALIGN")));
                }
            }
