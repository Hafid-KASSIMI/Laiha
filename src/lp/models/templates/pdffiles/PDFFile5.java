...
import lp.models.templates.PDFsMetrics.PDFFile5.*;
...
public class PDFFile5 extends PDFFile123456 {
    
    public PDFFile5() {
        super();
        tpl = tempaltesDir + "Template.5.pdf";
        pages[0] = new Page1();
        pages[1] = new Page2();
        pages[2] = new Page3();
        pages[3] = new Page4();
    ...
    protected void add1PageGroup(Group g, PDPage page, int start, int stuCount, BasePage metrics) {
        ...
        try {
            try (PDPageContentStream pcs = new PDPageContentStream(doc, page, PDPageContentStream.AppendMode.APPEND, true, true)) {
                ...
                placeString(metrics.getTitle(), pcs, Settings.PREF_TEMPORARY.get("TPL5_TITLE"));
                for ( int i = start, j = 0; i < stuCount; i++, j++ ) {
                    placeStudent(metrics.getStuRect(), pcs, g.getStudent(i), j, getAlignment(Settings.PREF_TEMPORARY.get("TPL5_NAMES_ALIGN")));
                }
            }
