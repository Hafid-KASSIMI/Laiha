...
import lp.models.templates.PDFsMetrics.PDFFile4.*;
import lp.models.templates.PDFsMetrics.StudentLARectangle;
import lp.util.ALIGNMENT;
import org.apache.pdfbox.pdmodel.PDPage;
import org.apache.pdfbox.pdmodel.PDPageContentStream;

public class PDFFile4 extends PDFFile123456 {
    
    public PDFFile4() {
        super();
        tpl = tempaltesDir + "Template.4.pdf";
        pages[0] = new Page1();
        pages[1] = new Page2();
        pages[2] = new Page3();
        pages[3] = new Page4();
    ...
    protected void add1PageGroup(Group g, PDPage page, int start, int stuCount, BasePage metrics) {
        boolean bold = Settings.PREF_TEMPORARY.get("TPL4_BOLDER_NAMES").equals("Y");
        ...
        try {
            try (PDPageContentStream pcs = new PDPageContentStream(doc, page, PDPageContentStream.AppendMode.APPEND, true, true)) {
                ...
                placeString(metrics.getYear(), pcs, Settings.SCHOOL_DB.getYear(), ALIGNMENT.RIGHT);
                placeString(metrics.getGroup(), pcs, g.getName());
                placeString(metrics.getTitle(), pcs, Settings.PREF_TEMPORARY.get("TPL4_TITLE"));
                placeString(metrics.getCustomMsg1(), pcs, Settings.PREF_TEMPORARY.get("TPL4_CUSTOM_STRING"));
                for ( int i = start, j = 0; i < stuCount; i++, j++ ) {
                    placeStudent(metrics.getStuRect(), pcs, g.getStudent(i), j, getAlignment(Settings.PREF_TEMPORARY.get("TPL4_NAMES_ALIGN")));
                }
            }
