...
import lp.models.templates.PDFsMetrics.PDFFile1.*;
...
public class PDFFile1 extends PDFFile123456 {
    
    public PDFFile1() {
        super();
        tpl = tempaltesDir + "Template.1.pdf";
        pages[0] = new Page1();
        pages[1] = new Page2();
        pages[2] = new Page3();
        pages[3] = new Page4();
    ...
    protected void add1PageGroup(Group g, PDPage page, int start, int stuCount, BasePage metrics) {
        boolean bold = Settings.PREF_TEMPORARY.get("TPL1_BOLDER_NAMES").equals("Y");
        String separator = Settings.PREF_TEMPORARY.get("TPL1_SCHOOL_INFOS_SEPARATOR");
        ...
        try {
            try (PDPageContentStream pcs = new PDPageContentStream(doc, page, PDPageContentStream.AppendMode.APPEND, true, true)) {
                wrapString(metrics.getSchoolInfos(), pcs, Settings.SCHOOL_DB.getAcademy() + separator + Settings.SCHOOL_DB.getDirection() + separator + Settings.SCHOOL_DB.getSchool());
                placeString(metrics.getYear(), pcs, Settings.SCHOOL_DB.getYear());
                placeBidirectionalString(metrics.getTitle(), pcs, g.getName(), Settings.PREF_TEMPORARY.get("TPL1_TITLE"));
                placeString(metrics.getCustomMsg1(), pcs, Settings.PREF_TEMPORARY.get("TPL1_CUSTOM_STRING"));
                for ( int i = start, j = 0; i < stuCount; i++, j++ ) {
                    placeStudent(metrics.getStuRect(), pcs, g.getStudent(i), j, getAlignment(Settings.PREF_TEMPORARY.get("TPL1_NAMES_ALIGN")));
                }
            }
