...
public class Workbook1 extends Workbook123456 {
    ...
    public Workbook1(){
        ...
        pla = "F";
        customStr = "G4";
        tpl = tempaltesDir + "Template.1.xlsx";
        firstRow = 5;
    }
    ...
    protected void add1PageGroup(Group g, XSSFSheet sht, int start, int stuCount) {
        ...
        XSSFCellStyle cs;
        XSSFCell xc;
        HorizontalAlignment ha = getAlignment(Settings.PREF_TEMPORARY.get("TPL1_NAMES_ALIGN"));
        ...
        sht.getRow(cr.getRow()).getCell(cr.getCol(), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK).setCellValue(Settings.PREF_TEMPORARY.get("TPL1_TITLE") + " " + g.getName());
        cr = new CellReference(customStr);
        sht.getRow(cr.getRow()).getCell(cr.getCol(), Row.MissingCellPolicy.CREATE_NULL_AS_BLANK).setCellValue(Settings.PREF_TEMPORARY.get("TPL1_CUSTOM_STRING"));
